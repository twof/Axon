...
OPENBCI_PORT = '/dev/tty.usbserial-DQ007SU3'
TEENSY_PORT = '/dev/cu.usbmodem1421'
TEENSY_ENABLED = True
...
class MIPlotter(object):
    ...
    def plot(self):
        ...
        for i in range(look_back):
            ...
            control_arr[i] = self.model.predict(features.reshape(1, -1))
        ...
        print("self out_sig", self.out_sig)
        if TEENSY_ENABLED:
            self.teensy.write(str(out_sig).encode())

        # for i in range(4):
            # self.controls[-40:, i], label=str(i+1))
        # print('aayyyyy lmao')
        # plot(sig)
        # plot(conv1)
        # plot(conv2)
